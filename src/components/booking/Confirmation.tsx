...
const Confirmation = ({ bookingData, onNewBooking }: ConfirmationProps) => {
  ...
  return (
    <div className="animate-fade-in-up max-w-lg mx-auto">
      {/* Receipt Card */}
      <div className="bg-card border border-border/60 rounded-lg shadow-elevated overflow-hidden">
        {/* Top accent */}
        ...
        <div className="px-6 py-5 space-y-4 text-sm">
          ...
          {/* Date & Time */}
          <div className="flex items-center gap-6 py-3 px-4 bg-muted/50 rounded-md">
          ...
          {notes && (
            <div className="pt-3 border-t border-border/40">
              ...
              <p className="text-xs text-foreground bg-muted/30 rounded-md p-2.5">{notes}</p>
            </div>
          )}
        </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
        <Button variant="default" size="lg" onClick={onNewBooking} className="w-full sm:w-auto gap-2">
          <span>{t.bookAnotherAppointment}</span>
          <ArrowRight className="w-4 h-4" />
        </Button>
        <Button variant="outline" size="lg" onClick={handleAddToCalendar} className="w-full sm:w-auto gap-2">
