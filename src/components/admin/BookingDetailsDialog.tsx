...
const BookingDetailsDialog = ({ booking, open, onOpenChange, onSaved, employees }: BookingDetailsDialogProps) => {
  ...
  const handleDelete = async () => {
    ...
    const confirmMsg = isSlovak 
      ? 'Naozaj chcete túto rezerváciu NATRVALO VYMAZAŤ z databázy? Táto akcia je nevratná.' 
      : 'Are you sure you want to PERMANENTLY DELETE this booking from the database? This action cannot be undone.';
    ...
    if (error) {
      toast.error(isSlovak ? 'Nepodarilo sa vymazať' : 'Failed to delete');
      return;
    }
    toast.success(isSlovak ? 'Rezervácia bola natrvalo odstránená' : 'Booking permanently deleted');
    onSaved?.();
    onOpenChange(false);
  };
                  ...
                  <>
                    <div className="flex gap-1.5 sm:gap-2">
                       <Button
                        ...
                        className="gap-1 sm:gap-1.5 text-orange-600 hover:text-orange-700 hover:bg-orange-50 text-xs sm:text-sm px-2 sm:px-3"
                      >
                        <X className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
                        {isSlovak ? 'Len zrušiť' : 'Just cancel'}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                      ...
                      >
                        ...
                        {isSlovak ? 'Vymazať navždy' : 'Delete forever'}
                      </Button>
                    </div>
                    <div className="flex gap-1.5 sm:gap-2">
                      <Button
