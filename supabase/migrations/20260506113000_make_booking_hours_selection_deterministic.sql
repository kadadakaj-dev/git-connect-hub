-- Migration: Make create_secure_booking opening-hours lookup deterministic
-- Date: 2026-05-06
-- Scope: Root-cause fix for FE/BE mismatch when multiple time_slots_config rows exist for same day.
--
-- No change to booking business policy:
-- - still uses time_slots_config as source of truth
-- - still enforces start >= opening and end <= closing
--
-- Change:
-- - replace non-deterministic day config SELECT with deterministic latest-row selection
--   ordered by created_at DESC NULLS LAST, id DESC
...
BEGIN
    ...
    -- This matches FE/edge runtime behavior and prevents stale-row selection.
