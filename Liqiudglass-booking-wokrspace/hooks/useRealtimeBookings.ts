...
export function useRealtimeBookings() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "bookings" }, (payload) => {
        ...
        setFeed((prev) =>
          [{ id: crypto.randomUUID(), type: "INSERT" as const, bookingId: next.id, status: next.status, at: new Date().toISOString() }, ...prev].slice(0, 20)
        );
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "bookings" }, (payload) => {
        ...
        setFeed((prev) =>
          [{ id: crypto.randomUUID(), type: "UPDATE" as const, bookingId: next.id, status: next.status, at: new Date().toISOString() }, ...prev].slice(0, 20)
        );
      })
      .subscribe();
  ...
  return { bookings, feed, bookingsToday, loading, error };
}
